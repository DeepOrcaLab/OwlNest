...
interface ToggleSidePanelMessage {
  data?: {
    ...
    /** Explicit windowId — used when sender is popup (no tab context) */
    windowId?: number
  }
  sender?: {
    tab?: {
...
export function createToggleSidePanelHandler({
  ...
  return (message: ToggleSidePanelMessage): Promise<ToggleSidePanelResult> => {
    ...
    // Resolve windowId: prefer data.windowId (popup passes this explicitly),
    // fall back to sender.tab.windowId (content script in a tab)
    const windowId = message.data?.windowId ?? message.sender?.tab?.windowId
    ...
    console.warn("[Background] Received toggleSidePanel, windowId:", windowId, "source:", message.data?.source)
    try {
      ...
      return Promise.resolve(openResult)
        .then(() => {
          console.warn("[Background] sidePanel.open succeeded, windowId:", windowId)
      ...
      console.error("[Background] sidePanel.open threw synchronously:", error)
      logger.error("Failed to open side panel", error)
      return Promise.resolve({ ok: false, reason: "toggle-failed" } as const)
    }
  }
